...
# --- Compute the significance table ---
...
# --- Per-threshold time series with CI bands (on the mean_chl) ---
    ...
    for ax, m in zip(axes, METRICS_BASE):
        ...
        if not row.empty:
            ...
            ax.plot(df["year"], trend_line, "--", color="#d55e00", linewidth=0.6,
                    label=f"Theil–Sen {tr['slope']:.2g}/yr ({tr['stars']})")
...
# Significance stars (above the maximum |slope| + error per group)
for j, m in enumerate(METRICS_BASE):
    for i, p in enumerate(THRESHOLD_PCTS):
        col = f"{m}_p{p}"
        r = df_trends[df_trends.metric == col]
        if r.empty:
            continue
        s = r.iloc[0]["slope"]
        e_hi = r.iloc[0]["slope_hi"] - s
        stars = r.iloc[0]["stars"]
        if stars != "n.s.":
            ax.text(xpos[j] + i * width, s + abs(e_hi) + (0.02 * (abs(s) + 1)),
                    stars, ha="center", va="bottom", fontsize=6, color="#222222")
...
disp = disp[["metric", "slope", "slope_lo", "slope_hi",
             "slope_pct_per_decade", "mk_tau", "mk_p", "mk_trend", "stars", "n"]]
disp.columns = ["metric", "slope", "slope_lo", "slope_hi",
...
ax_t.set_title("Multi-threshold trend significance (Theil–Sen + Mann–Kendall)",
               fontsize=7, pad=8)
plt.tight_layout()
